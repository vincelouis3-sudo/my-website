...
export default function GridDemo() {
  ...
  const renderCell = (col, row) => {
    switch (col.key) {
      case 'flag':
        return row.flag
          ? <img src={row.flag} alt={row.flagAlt || row.commonName} className="w-9 h-6 object-cover rounded shadow-sm border border-gray-200" />
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-screen-xl mx-auto px-4 py-6">
        ...
        {/* Toolbar */}
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 px-4 py-3 mb-4 flex items-center justify-between gap-3 flex-wrap">
            ...
            {hasFilters ? (
              <>
                ...
                <button
                  ...
                  className="px-2.5 py-1 rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 text-xs transition-colors"
                >
                  Clear all
                </button>
              </>
            ) : (
              <span className="text-gray-400 text-xs">Type in any column header below to filter · click a header to sort</span>
            )}
          </div>
          ...
          <div className="relative">
            <button
              ...
              className="px-3 py-1.5 text-sm rounded-lg border border-[#1e3a5f] text-[#1e3a5f] font-medium hover:bg-blue-50 transition-colors flex items-center gap-1.5"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {colPanelOpen && (
              <div className="absolute right-0 top-10 z-20 bg-white rounded-xl shadow-xl border border-gray-100 p-4 w-56">
                ...
                {ALL_COLUMNS.map(col => (
                  <label
                    key={col.key}
                    className={`flex items-center gap-2 py-1 text-sm select-none ${col.locked ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer hover:text-[#1e3a5f]'}`}
                  >
                    <input
                      type="checkbox"
        ...
        {!loading && error && (
          <div className="flex flex-col items-center justify-center py-32 gap-4 text-center">
            ...
            <p className="text-gray-500 text-sm max-w-sm">Could not reach the REST Countries API. Please check your connection and try refreshing.</p>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div className="px-4 py-3 bg-gray-50 border-t border-gray-100 flex flex-wrap items-center justify-between gap-3">
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-xs text-gray-400 hover:text-[#1e3a5f] hover:underline"
              >
